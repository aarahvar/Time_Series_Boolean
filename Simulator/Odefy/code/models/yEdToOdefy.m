...
try
    ...
    % maybe relative
    xDoc=xmlread(fullfile(pwd,file));
end
...
for i = 0:allNodes.getLength-1
    ...
    datanodes = getAllNamedChildren(thisNode,'data');
    for j=1:numel(datanodes)
        if ~isempty(getFirstNamedChild(datanodes{j},'y:ShapeNode'))
            datanode=datanodes{j};
        end
    end   
    labelnode = getFirstNamedChildSequence(datanode, {'y:ShapeNode','y:NodeLabel','#text'});
    ...
    if strcmp(cnodes{i+1,2},'&') || strcmp(cnodes{i+1,2},'&amp;')
        containsAnd=1;
    else
        cnodes{i+1,2} = validvarname(cnodes{i+1,2});
        nonAnd = nonAnd+1;
    end
...
for i = 0:allEdges.getLength-1
    ...
    % find the right data node
    datanodes = getAllNamedChildren(thisEdge,'data');
    for j=1:numel(datanodes)
        if ~isempty(getFirstNamedChild(datanodes{j},'y:PolyLineEdge'))
            datanode=datanodes{j};
        end
    end   
    % find arrow info
...
function list=getAllNamedChildren(node,name)
list = {};
checknode = node.getFirstChild;
while ~isempty(checknode)
    % no text nodes
...
function sib=getNextNamedSibling(node,name)

checknode = node;
while ~isempty(checknode)
    % no text nodes
    %    if (checknode.getNodeType == 1)
    if strcmp(checknode.getNextSibling,name)
        sib = checknode;
        break
    end
    %    end
